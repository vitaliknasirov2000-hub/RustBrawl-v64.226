...
const SetupMessaging = {
    init() {
        Interceptor.replace(Libg.offset(0xB12B28), new NativeCallback(function() {
            console.warn("[+][PepperCrypto::secretbox_open] Skipped decryption");
            return 1;
        }, 'int', []));


        Interceptor.attach(Libg.offset(0xB134D8), { // Messaging::sendPepperAuthentication
            onEnter(args) {
                this.messaging = args[0];
                console.warn("[+][PepperState::State][1] Pepper State Is", Memory.readU32(this.messaging.add(16)));
                Memory.writeU32(this.messaging.add(16), 5);
                args[1] = args[2];
                console.warn("[+][PepperState::State][2] Pepper State Is", Memory.readU32(this.messaging.add(16)));

            },
            onLeave(retval) {
                Memory.writeU32(this.messaging.add(16), 5);
                console.warn("[+][PepperState::State][3] Pepper State Is", Memory.readU32(this.messaging.add(16)));
            }
        });

        Interceptor.attach(Libg.offset(0xB13EA8), function() { // Messaging::encryptAndWrite
            this.context.r0 = 0x2774; // not tested
        });
    }
}
