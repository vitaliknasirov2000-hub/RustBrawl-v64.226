const Libg = {
    init() {
        Libg.module = Process.findModuleByName("NullsBrawl");
...
const SetupMessaging = {
    init() {
        Interceptor.replace(Libg.offset(0xB76CC8), new NativeCallback(function() {
            console.warn("[+][PepperCrypto::secretbox_open] Skipped decryption");
            return 1;
        }, 'int', []));


        Interceptor.attach(Libg.offset(0xB774C4), { // Messaging::sendPepperAuthentication
            onEnter(args) {
        ...
        Interceptor.attach(Libg.offset(0xB77BA8), function() { // Messaging::encryptAndWrite
